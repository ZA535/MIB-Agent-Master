...
namespace MIBAgent
{
    class Agent
    {
        ...
        public string Executioner(MySqlConnection con, string mac,Agent tm)
        {
            ...
            int boot_flag = 0;
            try { boot_flag = Convert.ToInt16(scal[1]); }catch(Exception e) { }

            int service_flag = 0;
            try { service_flag = Convert.ToInt16(scal[2]); } catch (Exception e) { }

            int kill_flag = 0;
            try { kill_flag = Convert.ToInt16(scal[3]); } catch (Exception e) { }

            int script_flag = 0;
            try { script_flag = Convert.ToInt16(scal[4]); } catch (Exception e) { }

            int port_flag = 0;
            try { port_flag = Convert.ToInt16(scal[5]); } catch (Exception e) { }
            ...
            try
            {
                portno = Convert.ToInt16(scal[10]);
            ...
            if (boot_flag == 1)
            {
                Console.WriteLine("Boot Flag");
                Console.WriteLine(boot_command);
                try
                { 
                boot_flag = 0;
        ...
        public string InitialUpdate(MySqlConnection con,string mac,string ip,string mask,string json)
        {
                ...
                else
                {
                    ...
                    cmd.CommandText = query;
                    cmd.ExecuteNonQuery();
                    Console.WriteLine("Ran Insert Into Execute");
                    return "0";
                }
